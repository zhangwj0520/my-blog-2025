...
### [ReactElement 对象](https://github.com/facebook/react/blob/v19.1.0/packages/react/src/jsx/ReactJSXElement.js#L176)

> 其 type 定义在[`shared`包中](https://github.com/facebook/react/blob/v19.1.0/packages/shared/ReactElementType.js).

所有采用`jsx`语法书写的节点, 都会被编译器转换, 最终会以`jsx()`的方式, 创建出来一个与之对应的`ReactElement`对象.
...
export type Fiber = {|
 //标记fiber的类型，即描述的组件类型
  ...
  // 如果是原生标签是dom,如果是类组件是实例,如果是函数组件是null
  stateNode: any,
  ...
  // 下一个兄弟fiber
  sibling: Fiber | null,
  //记录节点在当前层级中的位置下标,用于diff时,判断节点是否需要发生移动
  index: number,
...
- 这里的`fiber`树只是为了和上文中的`ReactElement`树对照, 所以只用观察红色虚线框内的节点. 根节点`HostRootFiber`在react 应用的启动模式章节中详细解读.
